...
class ShotEncoder(nn.Module):
    ...
    def forward(self, x):
        ...
        return feats.mean(dim=1)  # [B, 2048]
...
class CRNModel(nn.Module):
    ...
    def forward(self, x):
        ...
        return x.mean(dim=1)  # scene-level feature

# Prediction pipeline
def predict_scene_segments(shot_folder, model_path, output_path, similarity_threshold=0.85):
    ...
    for shot_id in shot_ids:
        ...
        image_tensor = torch.stack(frames).unsqueeze(0).to(device)
        with torch.no_grad():
            feat = model(image_tensor)  # [1, 2048]
        shot_features.append(feat.cpu().numpy()[0])
        valid_shot_ids.append(shot_id)

    # Clustering by cosine similarity
    scenes = []
    current_scene = [valid_shot_ids[0]]

    for i in range(1, len(valid_shot_ids)):
        sim = cosine_similarity(output.unsqueeze(0).cpu().numpy(), last_vector.unsqueeze(0).cpu().numpy())[0][0]

        if sim < similarity_threshold:
            scenes.append(current_scene)
            current_scene = []
        current_scene.append(valid_shot_ids[i])
    ...
    print(f"✅ Scene segmentation complete using cosine similarity. Saved to {output_path}")
